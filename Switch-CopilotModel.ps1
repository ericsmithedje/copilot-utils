...
.DESCRIPTION
    ...
    Dot-source this file into your PowerShell profile to make all functions
    ...
    FoundryLocal requires the 'foundry' CLI; the service is started on demand.
...
function Switch-CopilotModel {
    ...
    .DESCRIPTION
        ...
        currently available in FoundryLocal or Ollama. When -Model is provided,
    ...
    .PARAMETER Provider
        Optional provider hint when using -Model directly
        ('GitHub', 'LiteLLM', 'FoundryLocal', 'Ollama').
    .EXAMPLE
        Switch-CopilotModel
    .EXAMPLE
    ...
    if ($entries.Count -eq 0) {
        Write-Warning 'No providers available. Ensure LITELLM_BASE_URL is set, FoundryLocal is running, or Ollama has models pulled.'
        return
    }
    ...
    Set-CopilotModelEnvironmentVariables $selected
}

function Reset-CopilotModel {
    <#
    .SYNOPSIS
        Restores GitHub Copilot to the default GitHub-hosted provider.
    .DESCRIPTION
        Clears all BYOK environment variables from the current session,
        reverting to standard GitHub-hosted Copilot routing.
    .EXAMPLE
        Reset-CopilotModel
    #>
    [CmdletBinding()]
...
function Get-CopilotModel {
    ...
    if ([string]::IsNullOrWhiteSpace($baseUrl) -and [string]::IsNullOrWhiteSpace($model)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($apiKey)) {
            Write-Host "  API Key   : $(Get-MaskedApiKey $apiKey)" -ForegroundColor Green
        }
...
function Set-CopilotModelEnvironmentVariables {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        COPILOT_MODEL, and provider-specific vars based on the selected entry.
    .PARAMETER Entry
    ...
    if ($Entry.Provider -eq 'LiteLLM') {
        $masked = Get-MaskedApiKey $Entry.ApiKey
...
function Get-MaskedApiKey {
    ...
    .EXAMPLE
        Get-MaskedApiKey $env:LITELLM_API_KEY
    #>
    [CmdletBinding()]
    param(
        [string] $Key
...
Set-Alias -Name scm  -Value Switch-CopilotModel
Set-Alias -Name rcm  -Value Reset-CopilotModel
Set-Alias -Name gcm  -Value Get-CopilotModel
